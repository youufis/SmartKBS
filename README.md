...
> 智能点名、考勤统计、错题巩固、智能练习、课程练习、AI 资源推荐、学情分析、成长档案、AI 学伴、每周画像等功能。
> 基于 **FastAPI + React** 构建。
...
- **批量升年级**：管理员在「用户管理 → 批量操作」中一键预览并执行升年级，
  一年级→二年级→…→六年级→（毕业），初一→初二→初三→（毕业），高一→高二→高三→（毕业）
- **反向降级**：升年级的逆操作，高二→高一、高三→高二，与升年级互逆可反复执行
- **智能数据同步**：升/降级时可选择同步课堂积分（scores）和点名数据（rollcall）的年级归属
- **事务安全**：升/降级操作包裹在单事务中，失败自动全部回滚，数据零损失
...
- **在线增量升级**：管理员在「系统配置 → 版本管理」中一键检测版本并增量升级，
  自动 git fetch 增量拉取 → git reset 同步 → 数据库迁移 → 增量安装依赖 → 重启服务
- **智能 Git 环境诊断**：未安装 Git / 无 .git / 无 remote / 网络不通时给出具体中文提示和解决方案
- **备份 & 回滚**：升级前自动 git archive 备份，失败自动 git reflog 回滚，升级历史可追溯
- **同版本热修复检测**：版本检测接口先 git fetch 再统计落后提交，同版本内的文件更新也能被发现
- **后台自动检测新版本**：服务启动后每 6 小时自动轮询，发现更新直接通知所有管理员
- **升级详情展示变更文件**：升级历史中查看具体修改了哪些文件，不只是提交数

### v6.6.0 (2026-06-27)

🎨 **每日自我画像功能**

- **AI 自我画像**：基于通义万相模型，根据用户平台数据（积分、称号、学习/教学数据等）生成专属画像
- **15 种创意风格**：魔法学院、赛博学霸、国风墨韵、星际探险、热血漫画、童话精灵、蒸汽朋克等
- **15 种创意寄语体裁**：武侠、星际、古风等，大模型根据用户数据自动生成风格各异的创意寄语
- **角色感知**：根据用户角色（学生/教师/管理员）自动调整画像身份和寄语语气
- **个人画展**：历史画像按时间线展示，支持查看大图和寄语
- **分享画廊**：全校画廊、本班画廊、热门推荐三种分享模式，支持点赞互动
...
- **AI 生成 HTML 资源**：资源中心新增「AI 生成」功能，支持 4 种资源类型
  - **动画讲解**：用 3D 视差、粒子动画等技术制作生动有趣的知识讲解页
  - **互动答题**：卡片式交互答题页面，含选择题和即时反馈
  - **章节练习**：结构化章节练习题，含题号导航和提交按钮
  - **自定义 HTML**：自由输入主题，AI 按需生成任意 HTML 内容
- **主题选择系统**：每种资源类型提供多套预设主题（动画 17 种、答题 20 种、练习 5 种）
- **题库 + AI 混合出题**：生成时自动从题库中按关键词匹配题目，不足部分由 AI 补充新题
- **AI 新题自动入库**：AI 生成的新题目自动保存到题库，丰富题库资源
- **学科/年级智能关联**：根据当前所选学科和年级自动获取对应题库数据
