...
# S-GRADING(P3): 判分规则集中一处, 三个业务模块共用
from backend.grading_rules import (
    _ANS_SEP_RE, MAX_POINT_LEN,
    norm_text as _norm_text, num_tokens as _num_tokens, split_points as _split_points,
    point_hit as _point_hit, ratio_score as _ratio_score,
    grade_fill_by_rule as _grade_fill_by_rule, keyword_fallback as _keyword_rule,
)
...
# ── S-GRADING: 判分规则(要点切分/归一化/命中比例)已抽到 backend.grading_rules,
# 由同步练习、学习考试、随堂测验共用，避免三处各写一套兜底判分再各自漂移。
...
async def submit_practice(session_id: int, req: PracticeSubmitRequest, request: Request):
    ...
    def _keyword_fallback(q: dict, student_ans: str) -> dict:
        """AI 不可用/调用失败时的兜底判分（规则见 backend.grading_rules.keyword_fallback：
        按要点命中比例给分，切不出可比对要点则不猜分、标 needs_review 交教师）"""
        return _keyword_rule(q["correct_answer"], q["score"] or 10, student_ans)
