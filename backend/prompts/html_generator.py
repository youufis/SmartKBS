...
QUIZ_HTML_PROMPT = """你是一位专业的教育游戏 HTML 设计师。请根据以下知识点，生成一个**互动答题 HTML 页面**（课堂选班 → 随机点名 → 分组答题 → 积分结算 → 排行榜 → 错题解析）。
...
## ◈ ★★★ 功能模板（最高优先级：完整保留，只做指定修改）★★★

以下是一个**经过真实课堂验证、保证可运行**的互动答题页面完整模板（含全部 HTML/CSS/JavaScript）。
它已实现：年级/班级选择 → 随机点名 → 答题判分 → 积分结算 → 排行榜 → 错题解析，以及与后端 API 的完整交互。

**必须逐字保留（禁止任何改动）**：
1. HTML 中所有元素的 `id`（模板中出现的全部 id）
2. `<script>` 中除 `QUESTION_BANK` 外的**全部 JavaScript**（函数、变量、事件绑定、API 调用——这些代码已正确实现后端交互与异常降级）
3. 所有功能 CSS 类名及布局规则（.scene / .hidden / .option-btn / .opt-label / .feedback-box / .correct-fb / .wrong-fb / .btn-primary 等）

**只允许修改以下内容**：
1. `<title>` 和 `<h1>` 标题文字（结合知识点命名）
2. `:root` 中的 CSS 变量值（换成所选主题的配色；只改值，不删变量、不改类名）
3. `QUESTION_BANK` 数组中的题目数据（优先使用下方「题库提供的真实题目」；不足 10 道时按相同格式补充）
4. 可以在 `<body>` 中添加装饰元素（背景动画、光效、装饰图案等），但不得遮挡、删除或影响任何功能元素
...
<title>互动答题</title>
<!-- KaTeX（可选：加载失败不影响使用，代码中已有 safeRenderMath 保护） -->
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/katex@0.16.9/dist/katex.min.css">
<script defer src="https://cdn.jsdelivr.net/npm/katex@0.16.9/dist/katex.min.js"></script>
<script defer src="https://cdn.jsdelivr.net/npm/katex@0.16.9/dist/contrib/auto-render.min.js"></script>
<style>
/* ═════ 主题变量（只改值，换成所选主题配色，见下方主题章节） ═════ */
:root{
  --bg:#0a0a1a; --bg-card:rgba(255,255,255,.06); --border:rgba(255,255,255,.12);
  --text:#ffffff; --text-dim:rgba(255,255,255,.65);
  --primary:#00f0ff; --primary-dim:rgba(0,240,255,.3);
  --accent:#8833ff; --accent-dim:rgba(136,51,255,.25);
...
#app{position:relative;z-index:1;max-width:860px;margin:0 auto;padding:20px 16px 40px}
header{display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;padding:12px 18px;background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);margin-bottom:16px}
header h1{font-size:1.3rem;background:linear-gradient(90deg,var(--primary),var(--accent));-webkit-background-clip:text;background-clip:text;color:transparent;font-weight:800}
.round-indicator{display:flex;gap:8px}
.dot{width:12px;height:12px;border-radius:50%;background:var(--border);transition:all .3s}
.dot.done{background:var(--primary);opacity:.6}
.dot.active{background:var(--gold);box-shadow:0 0 10px var(--gold-dim);transform:scale(1.25)}
.scene{background:var(--bg-card);backdrop-filter:blur(14px);border:1px solid var(--border);border-radius:var(--radius);padding:26px 24px;box-shadow:0 8px 30px rgba(0,0,0,.35);animation:fadeIn .4s ease}
.hidden{display:none!important}
@keyframes fadeIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}
.scene-title{font-size:1.2rem;font-weight:800;text-align:center;margin-bottom:14px;color:var(--gold)}
.setup-group{max-width:440px;margin:0 auto}
.setup-row{display:flex;justify-content:space-between;align-items:center;gap:12px;margin:12px 0;flex-wrap:wrap}
.setup-row label{color:var(--primary);font-weight:600;min-width:90px}
select{padding:9px 12px;border-radius:var(--radius-sm);background:rgba(0,0,0,.35);border:1px solid var(--border);color:var(--text);outline:none;flex:1;min-width:150px}
select:disabled{opacity:.45}
.btn-primary{background:linear-gradient(135deg,var(--primary),var(--accent));border:none;color:#fff;font-weight:700;padding:11px 26px;border-radius:var(--radius-sm);cursor:pointer;transition:transform .2s,box-shadow .2s}
.btn-primary:hover:not(:disabled){transform:scale(1.04);box-shadow:0 0 14px var(--primary-dim)}
.btn-primary:disabled{opacity:.45;cursor:not-allowed;filter:grayscale(1)}
.pick-area{position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:330px;padding:10px 0}
.pick-ring{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);border-radius:50%;border:2px dashed var(--primary-dim);pointer-events:none}
.pick-ring:nth-child(1){width:230px;height:230px;animation:spin 10s linear infinite}
.pick-ring:nth-child(2){width:180px;height:180px;border-color:var(--accent-dim);animation:spinR 8s linear infinite}
.pick-ring:nth-child(3){width:130px;height:130px;border-color:var(--gold-dim);animation:spin 6s linear infinite}
@keyframes spin{to{transform:translate(-50%,-50%) rotate(360deg)}}
@keyframes spinR{to{transform:translate(-50%,-50%) rotate(-360deg)}}
.pick-name{font-size:3.4rem;font-weight:900;min-height:4.2rem;display:flex;align-items:center;justify-content:center;position:relative;z-index:2;text-shadow:0 0 18px var(--primary)}
.pick-name.rolling{animation:flick .1s infinite}
.pick-name.result{animation:popIn .5s cubic-bezier(.175,.885,.32,1.275);color:var(--gold)}
@keyframes flick{50%{opacity:.5}}
@keyframes popIn{from{transform:scale(.4);opacity:0}to{transform:scale(1);opacity:1}}
.pick-class-tag{text-align:center;color:var(--primary);margin:8px 0 14px;font-weight:600}
.pick-actions{display:flex;justify-content:center;gap:12px;flex-wrap:wrap}
.q-header{display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;margin-bottom:10px}
.q-number{color:var(--gold);font-weight:700}
.q-principle{background:var(--accent-dim);padding:4px 12px;border-radius:20px;font-size:.85rem}
.q-text{font-size:1.15rem;line-height:1.75;margin:12px 0;min-height:50px;background:rgba(0,0,0,.22);padding:16px 18px;border-radius:var(--radius-sm);border-left:4px solid var(--primary)}
.q-svg{margin:10px auto;text-align:center}
.q-svg svg{max-width:100%;height:auto}
.options-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:14px 0}
.option-btn{display:flex;align-items:flex-start;gap:10px;background:rgba(0,0,0,.3);border:1px solid var(--border);border-radius:var(--radius-sm);padding:13px 14px;text-align:left;cursor:pointer;color:var(--text);transition:all .2s;line-height:1.5}
.option-btn:hover:not(.disabled){border-color:var(--primary);background:var(--primary-dim)}
.opt-label{display:inline-flex;justify-content:center;align-items:center;width:26px;height:26px;border-radius:50%;background:var(--bg-card);border:1px solid var(--border);color:var(--primary);font-weight:700;flex-shrink:0}
.option-btn.selected{border-color:var(--primary);background:var(--primary-dim)}
.option-btn.correct{border-color:var(--green);background:rgba(0,255,136,.14);box-shadow:0 0 12px rgba(0,255,136,.3)}
.option-btn.correct .opt-label{background:var(--green);color:#062}
.option-btn.wrong{border-color:var(--red);background:rgba(255,51,85,.14);animation:shake .4s}
...
.feedback-box{display:none;margin-top:12px;padding:13px 15px;border-radius:var(--radius-sm);font-size:.95rem;line-height:1.7}
.feedback-box.show{display:block}
.correct-fb{background:rgba(0,255,136,.1);border-left:4px solid var(--green)}
.wrong-fb{background:rgba(255,51,85,.1);border-left:4px solid var(--red)}
.score-big{font-size:4rem;font-weight:900;text-align:center;color:var(--gold);margin:12px 0;animation:popIn .6s cubic-bezier(.68,-.55,.27,1.55)}
.score-detail{display:flex;justify-content:space-around;text-align:center;margin:12px 0;flex-wrap:wrap;gap:10px}
.score-stat{padding:8px 16px;background:rgba(0,0,0,.22);border-radius:var(--radius-sm);border:1px solid var(--border)}
.score-stat strong{font-size:1.25rem;color:var(--gold)}
.energy-wrap{height:20px;background:rgba(0,0,0,.35);border-radius:10px;overflow:hidden;margin:12px 0;border:1px solid var(--border)}
.energy-fill{height:100%;width:0;background:linear-gradient(90deg,var(--primary),var(--accent));transition:width 1s ease;position:relative}
.energy-fill::after{content:'';position:absolute;top:0;bottom:0;right:0;width:18px;background:rgba(255,255,255,.35);filter:blur(4px);animation:shine 1.5s infinite}
@keyframes shine{0%{opacity:0}50%{opacity:1}100%{opacity:0}}
.final-table{width:100%;border-collapse:collapse;margin:10px 0}
.final-table th,.final-table td{padding:10px;border-bottom:1px solid var(--border);text-align:center}
.final-table th{color:var(--primary)}
.rev-item{background:rgba(0,0,0,.25);padding:13px 15px;border-radius:var(--radius-sm);margin:10px 0;border-left:4px solid var(--red)}
.rev-student{color:var(--gold);font-weight:700;margin-bottom:6px}
.rev-opt{margin:4px 0;font-size:.92rem}
.rev-opt .right{color:var(--green);font-weight:700}
.rev-opt .user{color:var(--red);font-weight:700}
.toast{position:fixed;top:18px;left:50%;transform:translateX(-50%) translateY(-80px);background:var(--bg-card);backdrop-filter:blur(10px);border:1px solid var(--border);padding:11px 22px;border-radius:30px;z-index:999;transition:transform .3s;font-weight:600}
.toast.show{transform:translateX(-50%) translateY(0)}
.btn-row{display:flex;justify-content:center;gap:12px;flex-wrap:wrap;margin-top:14px}
@media(max-width:600px){.options-grid{grid-template-columns:1fr}.pick-name{font-size:2.2rem}.scene{padding:18px 14px}}
/* auto-generated by SmartKBS AI */
</style>
</head>
<body>
...
<div id="app">
  ...
  <div class="scene" id="sceneSetup">
    <div class="scene-title">⚙️ 课前配置</div>
    <div class="setup-group">
      <div class="setup-row"><label>🌍 选择年级</label><select id="gradeSelect"><option value="">加载中...</option></select></div>
      <div class="setup-row"><label>🏫 选择班级</label><select id="classSelect" disabled><option value="">-- 请先选择年级 --</option></select></div>
      <div class="setup-row" style="justify-content:center;"><button class="btn-primary" id="btnStart" disabled>⚡ 启动课堂互动</button></div>
    </div>
  </div>

  <!-- 场景 1：点名 -->
  <div class="scene hidden" id="scenePick">
    <div class="scene-title" id="pickTitle">🎯 第 1 组 · 随机点名</div>
    <div class="pick-area">
      <div class="pick-ring"></div><div class="pick-ring"></div><div class="pick-ring"></div>
      <div class="pick-name" id="pickName">READY?</div>
    </div>
    <div class="pick-class-tag" id="pickClassTag"></div>
    <div class="pick-actions">
      <button class="btn-primary" id="btnPickStart">🔄 开始抽取</button>
      <button class="btn-primary hidden" id="btnConfirm" style="background:var(--gold);color:#111;">✅ 确认答题</button>
      <button class="btn-primary hidden" id="btnRepick" style="background:var(--pink);">↻ 重抽</button>
    </div>
  </div>

  <!-- 场景 2：答题 -->
  <div class="scene hidden" id="sceneQuiz">
    <div class="q-header">
      <span class="q-number" id="qNumber">第1题/共2题</span>
      <span class="q-principle" id="qPrinciple">知识点</span>
    </div>
    <div class="q-text" id="qText">题目加载中...</div>
    <div class="q-svg" id="qSvgContainer"></div>
    <div class="options-grid" id="optionsGrid"></div>
    <div class="feedback-box" id="feedbackBox"></div>
    <div class="btn-row"><button class="btn-primary" id="btnNextQ" disabled>下一题 ➡️</button></div>
  </div>

  <!-- 场景 3：结算 -->
  <div class="scene hidden" id="sceneScore">
    <div class="scene-title">📊 阶段结算</div>
    <div class="score-big" id="scoreBig">+0</div>
    <div class="score-detail">
      <div class="score-stat">答对 <strong id="statCorrect">0</strong></div>
      <div class="score-stat">总题 <strong id="statTotal">2</strong></div>
      <div class="score-stat">积分 <strong id="statPoints">0</strong></div>
    </div>
    <div class="energy-wrap"><div class="energy-fill" id="energyFill"></div></div>
    <div class="btn-row"><button class="btn-primary" id="btnNextRound">进入下一组 🚀</button></div>
  </div>

  <!-- 场景 4：排行榜 -->
  <div class="scene hidden" id="sceneFinal">
    <div class="scene-title">🏆 课堂英雄榜</div>
    <table class="final-table"><thead><tr><th>#</th><th>姓名</th><th>班级</th><th>答对</th><th>积分</th></tr></thead><tbody id="finalBody"></tbody></table>
    <div id="reviewSection" class="hidden">
      <h3 style="color:var(--pink);margin:16px 0 10px;">📝 错题复盘</h3>
      <div id="reviewList"></div>
    </div>
    <div class="btn-row">
      <button class="btn-primary" id="btnReview" style="background:var(--pink);">📖 查看错题解析</button>
      <button class="btn-primary" id="btnRestart" style="background:transparent;border:1px solid var(--border);">🔄 重新开始</button>
    </div>
  </div>
</div>
...
/* ═══════ 题目数据（★ 仅替换此数组内容 ★） ═══════
   字段：question 题干；options 选项对象；answer 正确答案字母；
   explanation 解析；knowledge_point 知识点标签；difficulty 难度；
   svg_code 可选配图（SVG 字符串）；共需 10 题（5 组 × 2 题），不足时脚本自动循环补足 */
const QUESTION_BANK = [
  {id:1,type:"single",question:"题目文本",options:{A:"选项A",B:"选项B",C:"选项C",D:"选项D"},answer:"A",explanation:"解析文本",knowledge_point:"知识点标签",difficulty:"medium"}
];

/* ═══════ 全局状态 & 模拟数据（勿改） ═══════ */
let state = {grade:'', cls:'', student:'', round:0, qIdx:0, correct:0, startTime:0, students:[], scores:[], answers:[], wrongs:[]};
const GROUPS = [];
const MOCK_STUDENTS = ['张伟','李娜','王强','刘洋','陈静','杨帆','赵敏','黄磊','周杰','吴昊'];
const MOCK_CLASSES = ['高一(1)班','高一(2)班','高一(3)班'];
...
let toastTimer=null;
function showToast(icon,msg){const t=$('toast');t.textContent=icon+' '+msg;t.classList.add('show');clearTimeout(toastTimer);toastTimer=setTimeout(()=>t.classList.remove('show'),2200);}
/* KaTeX 安全渲染：CDN 未加载或异常时静默跳过，不影响答题 */
function safeRenderMath(el){try{if(typeof renderMathInElement==='function')renderMathInElement(el,{delimiters:[{left:'$$',right:'$$',display:true},{left:'$',right:'$',display:false}],throwOnError:false});}catch(e){}}
...
async function initGrades(){
  try{
    ...
    if(!Array.isArray(grades)||!grades.length)throw new Error('empty');
    $('gradeSelect').innerHTML='<option value="">请选择年级</option>'+grades.map(g=>'<option value="'+g+'">'+g+'</option>').join('');
  }catch(e){
    $('gradeSelect').innerHTML='<option value="">请选择年级</option>'+['高一','高二','高三'].map(g=>'<option value="'+g+'">'+g+'</option>').join('');
  }
}
async function loadClasses(){
  const g=$('gradeSelect').value;state.grade=g;
  const sel=$('classSelect');
  if(!g){sel.disabled=true;sel.innerHTML='<option value="">-- 请先选择年级 --</option>';refreshStart();return;}
  ...
  if(!Array.isArray(classes)||!classes.length)classes=MOCK_CLASSES;
  sel.innerHTML='<option value="">请选择班级</option>'+classes.map(c=>'<option value="'+c+'">'+c+'</option>').join('');
  sel.disabled=false;refreshStart();
}
async function loadStudents(){
  try{
    const r=await fetch('/api/rollcall/students?grade='+encodeURIComponent(state.grade)+'&class='+encodeURIComponent(state.cls)+'&teacher='+TEACHER);
    const data=await r.json();
    state.students=Array.isArray(data)?data.map(s=>s.name):[];
  }catch(e){state.students=[];}
}
async function apiPick(){
  try{
    const r=await fetch('/api/rollcall/pick',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({grade:state.grade,class:state.cls,teacher:TEACHER})});
    const d=await r.json();
    if(d&&!d.error&&d.student)return d.student;
  }catch(e){}
  const pool=state.students.length?state.students:MOCK_STUDENTS;
  return pool[Math.floor(Math.random()*pool.length)];
}
function apiMark(result){
  fetch('/api/rollcall/mark',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({grade:state.grade,class:state.cls,student:state.student,result:result,teacher:TEACHER})}).catch(()=>{});
}
function apiSaveScore(points){
  fetch('/api/scores/score',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({grade:state.grade,class:state.cls,name:state.student,points:points,teacher:TEACHER})}).then(r=>{if(r.ok)showToast('💰','积分已保存');}).catch(()=>{showToast('📝','本地模式');});
}
function apiSaveRecord(cc,total,pts){
  fetch('/api/rollcall/save-record',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({grade:state.grade,class:state.cls,student:state.student,type:'互动答题',title:document.title,correctCount:cc,totalQuestions:total,points:pts,answers:state.answers,teacher:TEACHER})}).catch(()=>{});
}

/* ═══════ 课堂流程（勿改） ═══════ */
function refreshStart(){$('btnStart').disabled=!($('gradeSelect').value&&$('classSelect').value);}
function prepareQuestions(){
  let qs=[...QUESTION_BANK];
  let i=0;
  while(qs.length<10){qs.push(JSON.parse(JSON.stringify(QUESTION_BANK[i%QUESTION_BANK.length])));i++;} // 题目不足时循环补足 10 道
  const shuffled=shuffle(qs.slice(0,10));
  for(let r=0;r<5;r++)GROUPS[r]=shuffled.slice(r*2,r*2+2);
}
function updateIndicator(){
  const dots=$('roundIndicator');dots.innerHTML='';
  for(let i=0;i<5;i++){const d=document.createElement('div');d.className='dot'+(i<state.round?' done':i===state.round?' active':'');dots.appendChild(d);}
}
function startRound(idx){
  state.round=idx;state.qIdx=0;state.correct=0;state.answers=[];state.student='';state.startTime=Date.now();
  updateIndicator();
  hide('sceneSetup');hide('sceneQuiz');hide('sceneScore');hide('sceneFinal');show('scenePick');
  $('pickTitle').textContent='🎯 第 '+(idx+1)+' 组 · 随机点名';
  $('pickName').textContent='READY?';$('pickName').className='pick-name';
  $('pickClassTag').textContent='';
  $('btnPickStart').classList.remove('hidden');$('btnPickStart').disabled=false;
  $('btnConfirm').classList.add('hidden');$('btnRepick').classList.add('hidden');
}
async function startPick(){
  $('btnPickStart').disabled=true;$('btnPickStart').classList.add('hidden');
  $('btnConfirm').classList.add('hidden');$('btnRepick').classList.add('hidden');
  const nameEl=$('pickName');nameEl.className='pick-name rolling';
  const rollIv=setInterval(()=>{nameEl.textContent=state.students.length?state.students[Math.floor(Math.random()*state.students.length)]:'...';},80);
  const student=await apiPick();
  clearInterval(rollIv);
  state.student=student;
  nameEl.textContent=student;nameEl.className='pick-name result';
  $('pickClassTag').textContent=state.grade+' · '+state.cls;
  $('btnConfirm').classList.remove('hidden');$('btnRepick').classList.remove('hidden');
}
function startQuiz(){hide('scenePick');show('sceneQuiz');showQuestion();}
function showQuestion(){
  const q=GROUPS[state.round][state.qIdx];
  $('qNumber').textContent='第'+(state.qIdx+1)+'题/共'+GROUPS[state.round].length+'题';
  $('qPrinciple').textContent=q.knowledge_point||'知识点';
  $('qText').innerHTML=q.question;
  $('qSvgContainer').innerHTML=q.svg_code?'<div class="q-svg">'+q.svg_code+'</div>':'';
  const grid=$('optionsGrid');grid.innerHTML='';
  Object.keys(q.options).forEach(k=>{
    const btn=document.createElement('button');btn.className='option-btn';
    btn.innerHTML='<span class="opt-label">'+k+'</span><span>'+q.options[k]+'</span>';
    btn.onclick=()=>handleAnswer(k);
    grid.appendChild(btn);
  });
  $('feedbackBox').className='feedback-box';$('btnNextQ').disabled=true;
  safeRenderMath($('qText'));
}
function handleAnswer(label){
  const q=GROUPS[state.round][state.qIdx];
  const isCorrect=label===q.answer;
  state.answers.push({question:q.question,options:q.options,correctAnswer:q.answer,yourAnswer:label,principle:q.knowledge_point||'',isCorrect:isCorrect});
  document.querySelectorAll('.option-btn').forEach(b=>{
    b.classList.add('disabled');
    const l=b.querySelector('.opt-label').textContent;
    if(l===q.answer)b.classList.add('correct');
    if(l===label&&!isCorrect)b.classList.add('wrong');
    if(l===label&&isCorrect)b.classList.add('selected');
  });
  const fb=$('feedbackBox');fb.classList.add('show');
  if(isCorrect){
    fb.className='feedback-box show correct-fb';
    fb.innerHTML='✅ <strong>回答正确！</strong><br>'+(q.explanation||'');
    state.correct++;
  }else{
    fb.className='feedback-box show wrong-fb';
    fb.innerHTML='❌ <strong>回答错误</strong>，正确答案：'+q.answer+'<br>'+(q.explanation||'');
    state.wrongs.push({student:state.student,question:q.question,options:q.options,userAns:label,correctAns:q.answer,explanation:q.explanation||'',principle:q.knowledge_point||''});
  }
  apiMark(isCorrect?'correct':'incorrect');
  $('btnNextQ').disabled=false;
}
function nextQuestion(){
  state.qIdx++;
  if(state.qIdx<GROUPS[state.round].length)showQuestion();
  else showScore();
}
function showScore(){
  const total=GROUPS[state.round].length;
  const points=state.correct===total?15:(state.correct>0?10:5); // 保底 5 分
  hide('sceneQuiz');show('sceneScore');
  $('scoreBig').textContent='+'+points;
  $('statCorrect').textContent=state.correct;
  $('statTotal').textContent=total;
  $('statPoints').textContent=points;
  setTimeout(()=>{$('energyFill').style.width=(state.correct/total*100)+'%';},100);
  state.scores.push({name:state.student,cls:state.cls,correct:state.correct,total:total,points:points});
  apiSaveScore(points);
  apiSaveRecord(state.correct,total,points);
  $('btnNextRound').textContent=state.round<4?'进入下一组 🚀':'查看排行榜 🏆';
}
function nextRound(){
  if(state.round<4)startRound(state.round+1);
  else showFinal();
}
function showFinal(){
  hide('sceneScore');show('sceneFinal');
  const tbody=$('finalBody');tbody.innerHTML='';
  [...state.scores].sort((a,b)=>b.points-a.points).forEach((s,i)=>{
    const tr=document.createElement('tr');
    tr.innerHTML='<td>'+(i+1)+'</td><td>'+s.name+'</td><td>'+s.cls+'</td><td>'+s.correct+'/'+s.total+'</td><td style="color:var(--gold);font-weight:700;">+'+s.points+'</td>';
    tbody.appendChild(tr);
  });
  if(state.wrongs.length===0){$('reviewSection').classList.add('hidden');$('btnReview').classList.add('hidden');}
  else{buildReview();$('reviewSection').classList.add('hidden');}
}
function buildReview(){
  ...
  state.wrongs.forEach(w=>{
    ...
    let opts='';
    Object.keys(w.options).forEach(k=>{opts+='<div class="rev-opt">'+k+'. '+w.options[k]+'</div>';});
    item.innerHTML='<div class="rev-student">👤 '+w.student+'</div>'
      +'<div style="margin-bottom:6px;"><strong>'+w.question+'</strong></div>'
      +opts
      +'<div class="rev-opt">你的答案：<span class="user">'+w.userAns+'</span>　正确答案：<span class="right">'+w.correctAns+'</span></div>'
      +'<div style="font-size:.85rem;color:var(--text-dim);margin-top:6px;">💡 '+(w.explanation||'')+'</div>';
...
function toggleReview(){
  ...
  if(sec.classList.contains('hidden')){sec.classList.remove('hidden');$('btnReview').textContent='📖 收起解析';}
  else{sec.classList.add('hidden');$('btnReview').textContent='📖 查看错题解析';}
}

/* ═══════ 事件绑定（保持 () => fn 写法，勿改） ═══════ */
$('gradeSelect').onchange=()=>loadClasses();
$('classSelect').onchange=()=>{state.cls=$('classSelect').value;refreshStart();loadStudents();};
$('btnStart').onclick=()=>{
  state.grade=$('gradeSelect').value;state.cls=$('classSelect').value;
  if(!state.grade||!state.cls){showToast('⚠️','请先选择年级和班级');return;}
  loadStudents();
  prepareQuestions();
  startRound(0);
};
$('btnPickStart').onclick=()=>startPick();
$('btnConfirm').onclick=()=>startQuiz();
$('btnRepick').onclick=()=>startPick();
...
**模板到此结束。请严格按照上面的模板生成，不要遗漏任何 id、函数或事件绑定。**

### 配图要求（在题目中可选）
- 题目可嵌入内联 SVG 示意图（`svg_code` 字段，SVG 应包含 viewBox、中文标注）
- 模板的 `showQuestion()` 已自动渲染 `svg_code`，无需额外代码

**重要 — 题目来源原则**：
1. 如果 prompt 中提供了「题库提供的真实题目」JSON 数据块，请**优先使用这些真实题目**嵌入 `QUESTION_BANK`，不要修改题目文本、选项、答案和解析
2. 如果真实题目数量不足 10 道，保留真实题目并补充新题目（新题目按同样数据格式添加）
3. 如果题目包含 `svg_code` 字段，请保留该字段（模板会自动渲染）
4. 如果题目或解析中包含 KaTeX 公式（$...$ 或 $$...$$），请保留 `$` 符号原样放入题目文本（模板的 `safeRenderMath()` 会自动渲染并保护）

## ◈ 可选视觉风格（共 20 种主题，选择其中一种使用）
**重要**：主题只影响**视觉样式**（配色、装饰、圆角、字体），**严禁**改动功能模板中的任何 HTML 结构、元素 id、JavaScript 逻辑和功能 CSS 类名。

### 主题套用方法（把所选主题的配色，套用到模板 `:root` 的语义变量上，只改值）
- `--bg` 页面背景（深色主题用深底色；浅色主题用浅底色）
- `--bg-card` 卡片/容器背景（深色主题用半透明白；浅色主题用白色）
- `--border` 边框色；`--text` 正文色；`--text-dim` 次要文字色
- `--primary` 主强调色（按钮渐变起点、高亮、题号）；`--primary-dim` 主色半透明
- `--accent` 次强调色（按钮渐变终点、知识点标签）；`--accent-dim` 次色半透明
- `--gold` 金色（标题、积分、抽中点名）；`--pink` 粉色（次要按钮、错题标题）
- `--green` 正确反馈色；`--red` 错误反馈色
- `--radius` 大圆角；`--radius-sm` 小圆角；`--font` 字体栈

示例（深色霓虹）：`--bg:#0a0a1a; --bg-card:rgba(255,255,255,.06); --primary:#00f0ff; --accent:#8833ff; --gold:#ffd700;`
示例（浅色森林）：`--bg:#f0f4e8; --bg-card:#ffffff; --primary:#2d7d46; --accent:#1a8b7a; --gold:#d4a017;`
示例（工业蓝图）：`--bg:#0d1b3e; --bg-card:rgba(20,40,80,.7); --primary:#5bc0eb; --accent:#d4a843; --gold:#d4a843;`
...
## ◈ 后端 API（模板已全部实现，请勿删改；此处仅作说明）

模板的 JavaScript 已完整实现以下后端交互，**无需新增任何 API 代码**：

1. `GET /api/rollcall/grades` → 年级列表（string[]）
2. `GET /api/rollcall/classes?grade={grade}&teacher={TEACHER}` → 班级列表（string[]）
3. `GET /api/rollcall/students?grade={grade}&class={class}&teacher={TEACHER}` → 学生列表（[{name,...}]）
4. `POST /api/rollcall/pick`，Body `{grade, class, teacher}` → 随机抽取学生，返回 `{student: 姓名}` 或 `{error}`
5. `POST /api/rollcall/mark`，Body `{grade, class, student, result, teacher}`，result 取 `'correct'` / `'incorrect'` / `'skip'`
6. `POST /api/scores/score`，Body `{grade, class, name, points, teacher}` → 保存积分
7. `POST /api/rollcall/save-record`，Body `{grade, class, student, type, title, correctCount, totalQuestions, points, answers, teacher}` → 保存答题记录

所有调用均已带异常降级：后端不可用时自动使用内置模拟数据继续运行，页面不会卡死。

## ◈ 硬性规则（违反任何一条 = 整页无法使用，必须遵守）

1. **禁止重复声明**：同一作用域内不得出现两个同名 const/let 变量（重复声明是语法错误，会导致整个脚本无法解析、页面完全不可用）
2. **禁止调用未定义函数**：JS 中出现的每个函数名都必须在模板中完整定义；不得删除模板中的任何函数或变量
3. **事件绑定必须包裹**：所有按钮/下拉框绑定必须写成 `$('xxx').onclick=()=>fn()` 或 `$('xxx').onchange=()=>fn()`；禁止写成 `$('xxx').onclick=fn`（事件对象会被当作参数传入函数，导致启动即报错）
4. **班级列表必须 API 加载**：禁止在 HTML 中硬编码班级名（班级必须由 `/api/rollcall/classes` 加载，模板已实现）
5. **抽取必须走后端**：点名必须调用 `POST /api/rollcall/pick`（模板已实现），禁止只做本地随机
6. **KaTeX 必须保护**：所有公式渲染必须通过模板中的 `safeRenderMath()`（带 typeof 判断），CDN 加载失败不得中断答题流程
7. **id 一致性**：JS 中 `$('id')` 引用的每个 id 必须在 HTML 中存在，禁止重命名模板中的任何 id
8. **保留降级逻辑**：所有 fetch 调用必须保留模板中的 try/catch 或 .catch() 降级处理
9. **生成后自查**：输出前逐项检查——① 模板中所有 id、函数、事件绑定是否完整保留；② QUESTION_BANK 是否含 10 道题且每题都有 question/options/answer/explanation；③ 是否只改了 :root 变量值和题目数据
10. **按钮禁用状态**：任何初始隐藏的按钮（style display:none）不得带 `disabled` 属性；JS 中显示按钮时必须同时确保其 enabled（`xxx.disabled=false`），否则用户无法点击

## ◈ 输出要求
- 输出**完整的 HTML 代码**（<!DOCTYPE html> 到 </html>），严格基于上方功能模板
- 所有 CSS 内嵌在 <style> 标签中，所有 JS 内嵌在 <script> 标签中
- 所有题目数据硬编码在 JS 的 QUESTION_BANK 中（10 道题，格式与模板一致）
- 年级/班级/学生数据通过 API 实时加载，模板已内置模拟数据作为降级方案
- 除 KaTeX（模板已带 safeRenderMath 保护）外，不引入任何外部 CDN / Google Fonts
- 纯 HTML 输出，不要加任何解释说明
- 在 <style> 末尾保留注释：/* auto-generated by SmartKBS AI */
"""
...
### ★ 优先方案：单文件 HTML（推荐）
**所有资源默认使用单文件 HTML 方式**，所有 CSS 内嵌 `<style>`、JS 内嵌 `<script>`。
单文件简单可靠，资源管理器直接可见，无需额外目录结构。

### 多文件输出（需要多个 HTML 文件支撑时使用）
如果交互实验的实现设计本身需要生成多个 HTML 文件来支撑
（如主界面 + 配置面板 + 帮助页面等独立 HTML），请使用多文件输出格式。
系统会自动解析并保存为子目录结构。
...
2. 主入口必须命名为 `index.html`
3. **优先全部内联在 `index.html` 中**，只有必要时才拆分
4. 图片文件（SVG 配图）不需要在输出中包含，系统会自动补充

**保存规则（了解即可）**：
- `index.html` → 直接放 HTML 目录，资源列表可见
- 配套文件 → 保存在同名子目录，如 `资源名/css/style.css`
"""
