...
# S-GRADING: 同题多份答案合并成一次调用评分（40 人 5 题: 200 次 → 25 次）
# 注意: 评分场景一律不注入技能 —— 技能段的"展示推导过程"等指令与"只输出 JSON"冲突
BATCH_SUBJECTIVE_GRADING_PROMPT = """你是严格的阅卷老师。下面是同一道题的参考答案与 {count} 份学生答案，请逐份独立评分。
...
## 满分
{max_score} 分。要点齐全且正确给满分；部分正确给部分分（{max_score} 的一半左右表示答对一半要点）；完全错误、空白或与题目无关给 0 分。

## 待评答案（共 {count} 份，编号 1 到 {count}）
{items}

## 评分要求
1. 只依据与参考答案的语义匹配程度、要点完整性与准确性评分，不受答案长短、礼貌用语、字迹排版影响
2. 相同质量的答案必须给相同分数，不要为了区分而人为拉开差距
3. 参考答案里的每一条要点都要单独核对，漏答的要点要扣分
4. 每份给一句 60 字以内的具体评语，指出命中与缺失的要点

## 输出格式（严格遵守）
只输出一个 JSON 数组，禁止任何解释文字、禁止 Markdown 代码块：
[{{"index": 1, "score": 分数, "comment": "评语"}}, {{"index": 2, "score": 分数, "comment": "评语"}}]
数组长度必须等于 {count}，index 必须覆盖 1 到 {count} 每个编号恰好一次，score 是 0 到 {max_score} 之间的数字。
